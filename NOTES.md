...
- **Collapsible chat panel** — Added expand/collapse toggle (▾/▴) in the header so users can minimize chat and see more of the map. Collapsed state keeps a compact status bar (connection dot + message count); expanded restores the full sheet/sidebar. Video, Ghost, and End actions stay in the expanded header; collapse auto-closes the emoji picker.
...
**How I added collapse:** Wrapped messages + compose in `.chat-panel__body` and toggled `.chat-panel--expanded` / `.chat-panel--collapsed` on the shell. CSS transitions animate `max-height` and layout — bottom sheet on mobile, floating compact card on desktop when collapsed — without unmounting chat state.
