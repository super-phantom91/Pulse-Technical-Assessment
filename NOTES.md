...
- **Connection UI stability** — Chat panel now opens when a request is **sent** (not only after accept), so the sheet does not pop in and blink on connect. Entrance animation runs once; expanded layout uses fixed `dvh`/`vh` sizing with no height transitions. Map tether waits until fully connected.
